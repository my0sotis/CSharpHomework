...
namespace Order
{
    public class Order
    {
        public Order(string num, string name, string pnum,uint cate)
        {
            OrderId = num;
            ClientName = name;
        ...
        public uint ProductCategory { set; get; }
        ...
        {
            if (this.ListOfDetails.Contains(details))
            {
                throw new Exception($"orderDetails-{details.ProductName} is already existed!");
            }
    ...
    {
        public OrderDetails() { }
        public OrderDetails(string name,int price,int num)
        {
            ProductName = name;
    ...
    public class OrderService
    {
        ...
        {
            Console.Write($"订单号:{order.OrderId}  ");
        ...
        //public void AddOrderFromConsole()
        //{
        ...
        //        s = Console.ReadLine();
        //    }
        //    newOrder.ProductCategory = uint.Parse(s);
        ...
        {
            int t = int.Parse(s);
            List<Order> orders = SearchByOrderId(t).ToList();
            foreach (Order o in orders)
            {
                if (o.OrderId == s)
                {
        ...
        //使用LINQ语句进行查询操作
        public List<Order> SearchByOrderId(long num)
        {
            var o = ListOfOrder.Where(a => long.Parse(a.OrderId) == num);
            return o.ToList();
        }
        public List<Order> SearchByOrderId()
        {
            return ListOfOrder;
        }
        ...
        public List<Order> SearchByWhichTotalPriceOver(long num)
        {
        ...
        public List<Order> Search()
        {
            ...
            switch (Flag)
            {
                case 1:
                    ...
                    orders = SearchByOrderId(int.Parse(s));
                    break;
                case 2:
                    Console.Write("请输入用户名:");
        ...
        {
            var o = ListOfOrder.Where(a => int.Parse(a.OrderId) == num);
        ...
        public void DelOrderByClientName(string name)
        {
            var o = ListOfOrder.Where(a => a.ClientName == name);
            List<Order> orders = o.ToList();
            foreach (Order t in orders)
